...
- (void)addNotis {
    [[NSNotificationCenter defaultCenter] addObserver:self
                                             selector:@selector(playDidPrepared:) name:kMoviePlayerDidPreparedNotification object:nil];
    [[NSNotificationCenter defaultCenter] addObserver:self
...
- (void)playDidPrepared:(NSNotification*)notification {
    NSLog(@"----->>>>>>>> playDidPrepared");
    weakify(self)
    fbmovie_gcd_main_async_safe(^{
        strongify(self)
        if (notification.userInfo && [notification.userInfo.allKeys containsObject:kMoviePlayerDidPreparedResultKey]) {
            FBMovieError errType = [[notification.userInfo objectForKey:kMoviePlayerDidPreparedResultKey] integerValue];
            NSLog(@"----------->>>>>>>>>>>>>> playDidPrepared result: %@", errorMessage(errType));
            if (errType != FBMovieErrorNone) {
                UIAlertController* alert = [UIAlertController alertControllerWithTitle:@"" message:@"解析失败" preferredStyle:UIAlertControllerStyleAlert];
