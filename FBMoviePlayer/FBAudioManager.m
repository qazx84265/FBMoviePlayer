...
@property (nonatomic, assign, readwrite) Float32           outputVolume;
@property (nonatomic, assign, readwrite) BOOL               playing;
@property (nonatomic, copy, readwrite) NSString   *audioRoute;
...
@implementation FBAudioManager

+(instancetype)audioManager {

    static FBAudioManager *instance = nil;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        instance = [[FBAudioManager alloc] init];
    });
    return instance;
}
...
-(BOOL)checkSessionProperties {
    ...
    NSLog(@"-------->>>>>>>>>> We've got %d output channels", (int)newNumChannels);
...
-(BOOL)renderFrames: (UInt32) numFrames
    ...
    if (_playing && _outputHandler ) {
        
        // Collect data to render from the callbacks
        _outputHandler(_outData, numFrames, _numOutputChannels);
                                ...
                                AudioBufferList                * ioData)
{
    ...
    FBAudioManager *am = (__bridge FBAudioManager *)inRefCon;
